...
int main(int argc, char *argv[]){
  ...
  petuum::ServerProxy<int> *proxy 
    = new petuum::ServerProxy<int>(comm);  
  pthread_barrier_t *barrier = new pthread_barrier_t();
  pthread_barrier_init(barrier, NULL, num_thrs + 1);

  petuum::ServerThreadInfo<int> *thrinfo = new petuum::ServerThreadInfo<int>();
  ...
  for(int i = 0; i < num_thrs; ++i){
    suc = pthread_create(thr + i, NULL, petuum::ServerProxy<int>::ThreadMain,
        thrinfo);
