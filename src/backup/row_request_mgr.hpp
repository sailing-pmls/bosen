...
// updates. The local version number denotes the latest version of updates
...
public:
  ...
  // number and no actual request is sent.
  ...
  // Corresponding row requests are removed upon returning.  If all row
  // requests prior to some version are removed, those OpLogs are removed as
  // well. app_thread_ids are id of the threads whose request is satisfied
  // with this reply.
