...
DEFINE_double(subsampling, 1, "Subsampling ratio");
DEFINE_int32(label_range, -1, "range of labels, [0, label_range]");
...
int main(int argc, char** argv) {
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    // subsampling
    if (FLAGS_label_range >= 0 && lines[line_id].second > FLAGS_label_range) {
      continue;
    }
    int rand_num = rand() % 100;
    if (rand_num > (FLAGS_subsampling * 100)) {
      continue;
    }
