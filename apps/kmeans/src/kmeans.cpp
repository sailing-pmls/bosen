...
void KMeans::Start() {
	...
	config.assignment_output_location_ = context.get_string("output_assignments_folder");
	...
	for (int epoch = 0; epoch < num_epochs; epoch++) {
		...
		//LOG(INFO)<<"clientid:  "<< client_id <<"  thread id: " << thread_id << "  " << "epoch: " << epoch << "   Objective: " <<workerThread.ComputeObjective();
		petuum::PSTableGroup::Clock();

		workerThread.RefreshParams();
//		workerThread.PushObjective(epoch, num_epochs+1);
//		if(epoch%10==0){
//			workerThread.ClearCentersCount();
//		}

//		LOG(INFO) << "number of ccenters " << workerThread.ComputeNumberOfPointsObserved() ;
	}
...
//			LOG(INFO) << "Objective Value after epoch " << i << " is "  << tmp_row[i];
//		}
		...
		FILE * output = fopen(context.get_string("output_centers_file").c_str(), "w");

		for(int i=0;i<num_centers;i++) {
			fprintf(output, "%s\n", c.getCenterAt(i).AsString().c_str());
		}
		fclose(output);
	}
