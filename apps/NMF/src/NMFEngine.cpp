    ...
    void NMFEngine::SaveResults(int thread_id, petuum::Table<float> & R_table, 
        ...
        if (client_id_ == 0 && thread_id == 0) {
            ...
            if (output_data_format_ == "text") {
                ...
                std::string R_filename = output_path_ + "/R.bin";
                fout_R.open(R_filename.c_str(), std::ios::binary);
            } else {
            ...
            for (int row_id = 0; row_id < rank_; ++row_id) {
                ...
                for (int col_id = 0; col_id < m; ++col_id) {
                    ...
                    if (output_data_format_ == "text") {
                        ...
                        fout_R.write(reinterpret_cast<char*> ( 
                                &(R_row_cache[col_id])), 4);
                    }
                }
                if (output_data_format_ == "text") {
                    fout_R << "\n";
        ...
        if (thread_id == 0) {
            if (output_data_format_ == "text") {
                ...
                std::string L_filename = output_path_ + "/L.bin." 
                    + std::to_string(client_id_);
                fout_L.open(L_filename.c_str(), std::ios::binary);
            } else {
                LOG(FATAL) << "Unrecognized data format: " << output_data_format_;
            }
                ...
                if (L_matrix_loader_.GetCol(col_id_client, L_cache)) {
                    for (int row_id = 0; row_id < rank_; ++row_id) {
                        ...
                        if (output_data_format_ == "text") {
                            ...
                            fout_L.write(reinterpret_cast<char*> (
                                    &(L_cache[row_id])), 4);
                        }
                    }
                        if (output_data_format_ == "text") {
            ...
            for (int row_id = 0; row_id < rank_; ++row_id) {
                ...
                for (int col_id = 0; col_id < m; ++col_id) {
                    if (input_data_format_ == "text") {
                        ...
                        fout_R.read(reinterpret_cast<char*> (
                                &(R_row_cache[col_id])), 4);
                    }
                   ...
                   if (L_matrix_loader_.GetCol(col_id_client, L_cache)) {
                       for (int row_id = 0; row_id < rank_; 
                        ...
                        if (input_data_format_ == "text") {
                            ...
                            fout_L.read(reinterpret_cast<char*> (
                                &(L_inc_cache[row_id])), 4);
                        }
                        L_inc_cache[row_id] = 
