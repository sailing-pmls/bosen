...
class Net {
 public:
  ...
  explicit Net(const NetParameter& param, const int thread_id, 
      const int net_id);
  explicit Net(const string& param_file, const int thread_id, 
      const int net_id);
  virtual ~Net() {}
  ...
  const vector<int>& param_owners() { return param_owners_; }
