...
struct thread_args {
  ...
  tbb::concurrent_hash_map<KType, ValType>& table;
  cacheint* reads;
  bool in_table;
...
void read_thread(thread_args<KType> rt_args) {
  ...
  tbb::concurrent_hash_map<KType, ValType>& table = rt_args.table;
  ...
  while (true) {
    for (auto it = begin; it != end; it++) {
      ...
      typename tbb::concurrent_hash_map<KType, ValType>::const_accessor read_acc;
      ASSERT_EQ(table.find(read_acc, *begin), in_table);
      reads->num++;
    }
    if (finished->load(std::memory_order_acquire)) {
      return;
...
void insert_thread(thread_args<KType> it_args) {
  ...
  tbb::concurrent_hash_map<KType, ValType>& table = it_args.table;
  for (;begin != end; begin++) {
    /*
    ...
    */
    typename tbb::concurrent_hash_map<KType, ValType>::accessor write_acc;
    ASSERT_TRUE(table.insert(write_acc, *begin));
    // Set the value.
    write_acc->second = 0;
  }
}
...
class ReadEnvironment {
  public:
    ...
    ReadEnvironment()
      : table_dummy(power), numkeys(table_dummy.bucket_count()*SLOT_PER_BUCKET), keys(numkeys) {
        ...
        // DW: Bug! This only allows KType to be integer.
    ...
    cuckoohash_map<KType, ValType> table_dummy;
    tbb::concurrent_hash_map<KType, ValType> table;
    size_t numkeys;
