...
void insert_thread(tbb::concurrent_hash_map<KType, ValType>& table,
...
class InsertEnvironment {
  public:
    ...
    InsertEnvironment()
      : table_dummy(power),
        numkeys(table_dummy.bucket_count()*SLOT_PER_BUCKET), keys(numkeys) {
...
void InsertThroughputTest(InsertEnvironment<KType> *env) {
  ...
  std::cout << "(with " << thread_num << " threads)." << std::endl;
